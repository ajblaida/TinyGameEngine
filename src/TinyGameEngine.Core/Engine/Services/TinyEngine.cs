...
public abstract class TinyEngine : IGameEngine, IDisposable
{
    ...
    public Task<GameState?> GetGameStateAsync(string gameId, string? playerId, CancellationToken cancellationToken = default)
    {
        try
        {
            return _gameStateService.LoadGameStateAsync(gameId, cancellationToken)
                .ContinueWith(task =>
                {
                    if (task.IsFaulted)
                    {
                        _logger.LogError(task.Exception, "Failed to load game state for {GameId}", gameId);
                        throw task.Exception!;
                    }

                    var state = task.Result;
                    if (state == null || (playerId != null && state.PlayerId != playerId))
                    {
                        _logger.LogWarning("Game state not found or player mismatch for {GameId}", gameId);
                        return null;
                    }
        ...
        {
            _logger.LogError(ex, "Error retrieving game state for {GameId}", gameId);
            throw;
        }
    }
}
